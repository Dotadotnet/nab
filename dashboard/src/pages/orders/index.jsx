...
const ListOrder = () => {
  ...
  useEffect(() => {
    ...
    if (error?.data) {
      toast.error(error?.data?.message, { id: "order-loading" });
    ...
    if (deleteOrder && !isRemoving) {
      toast.success("سفارشات با موفقیت حذف شد.", { id: "order-remove" });
    }
        ...
        {isLoading || (orders && orders.length == 0) ? (
          ...
          orders.map((order) => {
            const { title, description } =
              order?.translations[0].translation?.fields || {};
            return (
              <div
                key={order._id}
              ...
              >
                <div className="col-span-10 lg:col-span-3 text-center flex items-center">
                  <StatusIndicator isActive={order.status === "active"} />
                ...
                <div className="hidden lg:col-span-2 col-span-5 gap-2 text-right lg:flex justify-left items-center">
                  <article className="flex-col flex gap-y-2">
                    <span className="flex text-right">{order.slug}</span>
                  </article>
                </div>
                ...
                <div className="col-span-2 md:col-span-1 gap-2 text-center flex justify-center items-center">
                  <article className="lg:flex-row flex flex-col justify-center gap-x-2  gap-y-2">
                    <DeleteModal
                      message="آیا از حذف این سفارشات اطمینان دارید؟"
