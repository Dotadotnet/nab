...
import { useGetFeaturedProductQuery, useUpdateFeaturedProductMutation } from "@/services/featuredProduct/featuredProductApi";
...
import { setFeaturedProduct } from "@/features/featuredProduct/featuredProductSlice";
...
const UpdateFeaturedProduct = ({ id }) => {
  ...
  const {
    ...
    data: fetchData,
    error: fetchError,
  } = useGetFeaturedProductQuery(id, { skip: !isOpen });
  const [
  ...
  useEffect(() => {
    if (isUpdateing) {
      toast.loading("در حال به‌روزرسانی ...", {
        id: "fetchFeaturedProduct",
      });
    }
    console.log(fetchData);
    if (fetchData) {
      toast.success(fetchData?.message, { id: "fetchFeaturedProduct" });
    }

    if (fetchError?.data) {
      toast.error(fetchError?.data?.message, { id: "fetchFeaturedProduct" });
    }

    if (updateData) {
      toast.success(updateData?.message, { id: "updateFeaturedProduct" });
      setIsOpen(false);
    }

    if (updateError?.data) {
      toast.error(updateError?.data?.message, { id: "updateFeaturedProduct" });
    }
  }, [fetching, fetchData, fetchError, isUpdateing, updateData, updateError]);

  useEffect(() => {
    if (fetchData) {
      dispatch(setFeaturedProduct(fetchData?.data));
  ...
  const handleUpdateUser = (data) => {
    ...
    updateFeaturedProduct({ id: id, body: formData });
  };
