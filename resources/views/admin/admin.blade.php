...
@section('main')
    ...
    <form enctype="multipart/form-data" method="POST"
        action="@if ($status == 'edite') {{ route('admin.update', ['admin' => $data->id]) }}@elseif($status == 'create'){{ route('admin.store') }} @endif"
        autocomplete="off">
        @csrf
        @if ($status == 'edite')
            @method('PUT')
        @endif
        @php
            $inputs = [['name' => 'name', 'text' => 'اسم ادمین', 'type' => 'fa'], ['name' => 'last_name', 'text' => 'نام خانوادگی ادمین', 'type' => 'fa'], ['name' => 'meli_code', 'text' => 'کد ملی ادمین', 'type' => 'num', 'maxlen' => '10'], ['name' => 'phone', 'text' => 'شماره ادمین', 'type' => 'num', 'maxlen' => '11'], ['name' => 'password', 'text' => 'پسورد ادمین (اختیاری)', 'type' => 'en-num']];
        @endphp
        @if ($status == 'edite')
