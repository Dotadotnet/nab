...
let password_div = document.querySelector('div.password-div');
let tornoment_div = document.querySelector('h3.tornoment-div');
if (password_div) {
    let button_show = password_div.querySelector('span');
    let input_password = password_div.querySelector('input');
...
let button_submit_register = document.querySelector('button.save-register');
let button_forgot_password = document.querySelector('button.forgot-password');
        ...
        if (!error) {
            if (hash_string == 'submited') {
                animateCSS(info_div, 'fadeOutLeft').then((message) => {
                    info_div === null || info_div === void 0 ? void 0 : info_div.classList.add('hidden');
                });
                accept_div === null || accept_div === void 0 ? void 0 : accept_div.classList.remove('hidden');
                animateCSS(accept_div, 'fadeInRight');
            }
            if (!hash_string) {
                animateCSS(info_div, 'fadeInLeft');
                animateCSS(accept_div, 'fadeOutRight').then((message) => {
                    accept_div === null || accept_div === void 0 ? void 0 : accept_div.classList.add('hidden');
                    location.reload();
                });
            }
        }
...
// function_onchange();
const inputs_code = document.querySelectorAll('input.code');
const input_verifys = document.querySelectorAll('div.input-verify');
...
const redirect_links = document.querySelectorAll('a.redirect-link');
