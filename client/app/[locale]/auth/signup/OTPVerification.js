...
export default function OTPVerification({ phone, onSuccess, resendCode }) {
  ...
  const handleVerify = async (e) => {
    ...
    if (code.length !== 4) {
      toast.error(l("PleaseEnterVarifyCode"));
      return;
    }
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading(l("VarfyinYourCode"), { id: "verify" });
    }
        ...
        <div className="max-w-md mx-auto text-center sm:px-8">
          <header className="mb-8">
            <h1 className="text-2xl dark:text-gray-100 mb-1">
              {l("ConfirmMobilePhone")}
            </h1>
            <p className="text-[15px] text-slate-500">
              {l("VarifyCodeSendedYourPhone" , { 0 : phone})}
            </p>
          </header>
