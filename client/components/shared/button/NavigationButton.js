...
const NavigationButton = ({ direction = "next", onClick }) => {
  ...
  return (
    ...
    >
      {isNext ? (
        <>
          <HiChevronRight className="h-6 w-6 transition-transform duration-300 transform group-hover:translate-x-1 group-focus:translate-x-1" />
          <span className="!mr-2">بعدی</span>
        </>
      ) : (
        <>
          <span className="ml-2">قبلی</span>
          <HiChevronLeft className="h-6 w-6 transition-transform duration-300 transform group-hover:!-translate-x-1 group-focus:!-translate-x-1" />
        </>
      )}
    </button>
  );
