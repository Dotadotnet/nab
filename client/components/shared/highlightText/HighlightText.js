...
const HighlightText = ({ title, center = false, tag = null }) => {
  ...
  return (
    <>
      {
        tag ?
          <div className={`relative inline ${center ? "text-center" : "text-right"}`}>
            <motion.h1
              initial={{ opacity: 0, x: 10 }}
              whileInView={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.3, ease: "easeOut" }}
              viewport={{ once: false, amount: 0.5 }}
              className={`text-headingColor inline   justify-start items-center gap-x-2  md:text-4xl text-4xl fld capitalize`}
            >
              {title}
            </motion.h1>
            <motion.span
